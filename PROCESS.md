...
### 6주차 (6/5~6/11) — Intra-paper QA ✅ (완료: 2026-06-22)
...
- `POST /api/chat` — `{ message, paper, history }` 수신. 논문 로드 시 논문 전용 시스템 프롬프트(범위 제한 + 섹션 참조 지시) 적용, 미로드 시 일반 의학 튜터 fallback. `history` 배열로 멀티턴 대화 지원
- `app/page.tsx` — `handleSendMessage()`에서 `loadedPaper.paper` + `chatHistory` 전달. QA 탭 빈 상태 안내 문구를 논문 로드 여부에 따라 분기

---

### 7주차 (6/12~6/18) — 통합 + UI 정리 🎯 핵심 체크포인트 ✅ (완료: 2026-06-22)
...
- 탭 순서 재정렬: Q&A|번역|통계|배경지식 → **배경지식|번역|통계|Q&A** (학습 흐름 순서)
- 논문 로드/전환 시 배경지식 탭 자동 포커스 + QA 채팅 기록 초기화
- 논문 미로드 상태에서 탭 버튼 `disabled` 처리 (시각적 비활성화)

---

### 8주차 (6/19~6/25) — P1 선택 기능 1: 퀴즈 + 신뢰도 카드 ✅ (완료: 2026-06-22)

- 퀴즈: 논문 기반 객관식 / 참거짓 문제 자동 생성
- 신뢰도 카드: 연구설계·표본수·저널 정보 시각화
- **완성 기준:** P1 기능 작동 ✅ (둘 다 구현)

**산출물**

- `POST /api/reliability` — 논문 메타데이터 → `{ cards: [{ category, value, interpretation, level }], overall }` (높음/보통/낮음/정보없음 레벨)
- `POST /api/quiz` — 논문 본문 → `{ questions: [{ question, options[], answer, explanation }] }` (4지선다 5문제)
- `app/page.tsx` — 신뢰도·퀴즈 탭 lazy 추가, 탭 6개 overflow 스크롤, 퀴즈 즉시 채점 UI, 논문 전환 시 답안 초기화

---

### 9주차 (6/26~7/2) — P1 선택 기능 2: 시각화 ✅ (완료: 2026-06-22)
...
- `POST /api/visualize` — 논문 본문 → Mermaid flowchart 코드 + 설명. 시스템 프롬프트 수정만으로 다이어그램 타입·구조 변경 가능
- `app/components/MermaidDiagram.tsx` — `mermaid.render()` 클라이언트 컴포넌트 (SSR 우회), 렌더링 실패 시 raw 코드 폴백
- `app/page.tsx` — 시각화 탭 lazy 추가, Mermaid 코드 토글 보기
...
| 6 | QA 기능 작동 | ✅ 2026-06-22 완료 |
| 7 | P0 6개 기능 모두 연결된 프로토타입 | ✅ 2026-06-22 완료 |
| 8 | 퀴즈 + 신뢰도 카드 추가 | ✅ 2026-06-22 완료 |
| 9 | 시각화 추가 | ✅ 2026-06-22 완료 |
| 10 | 안정화된 버전 | — |
| 11 | 테스트 결과 보고서 | — |
| 12 | 최종 산출물 일체 | — |
