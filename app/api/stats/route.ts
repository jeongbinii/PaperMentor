...
- 오직 제공된 [초록]과 [본문 발췌]에 실제로 등장하는 수치만 추출합니다. 본문에 없는 수치를 추정·계산·생성하지 말고, 논문이 유명하더라도 기억·사전지식으로 수치(I²·HR·p값 등)를 보충하지 마십시오. 제공된 텍스트에 없는 수치는 존재하지 않는 것으로 취급합니다.
...
[초심자 풀어쓰기] metric·interpretation·clinicalMeaning에서 낯선 생물학·실험 약어·표기가 처음 나오면 그 자리에서 한 구절로 뜻을 덧붙여, 그 항목만 읽어도 초심자가 이해되게 합니다.
- 예: "NVT(neovascular tuft — 망막 표면 밖으로 자라는 비정상 신생혈관 다발)", "AAV(유전자를 세포에 전달하는 데 쓰는 바이러스 벡터)", "Cdh5-Cre Sema3G fl/fl(내피세포에서만 Sema3G를 없앤 유전자 결손 마우스)", "HRE(저산소 반응요소 — 전사인자가 붙는 DNA 부위)".
- 단, value 필드는 본문 표기 그대로 둡니다(수치·원문 보존). 뜻풀이는 널리 알려진 표준 지식만 쓰고, 논문에 없는 사실·수치는 지어내지 마십시오. 뜻풀이는 핵심 용어에만 짧게 답니다.
...
- 연구설계가 관찰연구(코호트·환자대조·단면)이거나 관찰연구를 종합한 메타분석이면, 효과크기(핵심) 항목의 caution에 "무작위배정이 아니므로 인과관계로 단정할 수 없다"는 취지를 기본으로 적습니다.
