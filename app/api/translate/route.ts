...
[이 탭의 범위 — 임상 의학 도메인어만]
이 탭은 "임상 의학 도메인 고유어"만 다룹니다. 연구방법론·연구설계·통계·평가도구 개념은 다른 탭(배경지식·통계)이 전담하므로 여기서는 제외합니다.

[의학용어 판정 — 2단계로 판단]
무엇을 용어로 볼지는 "전문적으로 보이는가"가 아니라 아래 2단계로 판정합니다.

1단계 — 후보 게이트(임상 의학 도메인어인가):
  "의학사전(표준 의학용어집·MeSH 등)에 등재될 만한 임상 도메인 고유어 또는 그 표준 약어"이면 후보입니다.
  ...
  · 연구방법론·연구설계·평가도구·통계 용어: systematic review, meta-analysis, umbrella review, RCT, cohort, GRADE, AMSTAR, PRISMA, heterogeneity, effect size, odds ratio, confidence interval 등 → 이 탭에서 제외합니다(배경지식·통계 탭 소관). 본문이 방법론 위주라 임상 도메인어가 적으면 적게, 없으면 빈 배열로 둡니다.
...
function extractJson(text: string): TranslationResult {
  ...
  const parsed = (looseParse(slice) ?? {}) as Partial<TranslationResult>;
  return {
